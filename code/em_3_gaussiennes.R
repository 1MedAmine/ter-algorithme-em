# ALGORITHME EM — Mélange de 3 gaussiennes
# alpha=(0.4, 0.4, 0.2)  mu=(-2, 0, 2)  var=(0.3, 0.2, 0.2)
...
estm_obs = function(n) {
  ...
  for (i in 1:n) {
    u = runif(1)
    if (u < 0.4) {
      X[i] = rnorm(1, mean = -2, sd = sqrt(0.3))
    } else if (u < 0.8) {
      X[i] = rnorm(1, mean = 0, sd = sqrt(0.2))
    } else {
      X[i] = rnorm(1, mean = 2, sd = sqrt(0.2))
    }
  }
  return(X)
}
...
## Boucle EM avec stockage à itération 1, puis tous les 10
alpha_est = rep(1/K, K)
mu_est    = c(-1.5, 0.2, 1.8)
var_est   = rep(0.5, K)
...
for (iter in 1:200) {
  ...
  # On stocke à l'itération 1, puis tous les "pas" itérations
  if (iter == 1 || iter %% pas == 0) {
    alpha_list[[compteur]] = alpha_est
    mu_list[[compteur]]    = mu_est
    var_list[[compteur]]   = var_est
    iter_stock[compteur]   = iter
    compteur = compteur + 1
  }
  
  if (iter > 1 && max(abs(alpha_est - alpha_old)) < 1e-6) {
    cat("Convergence à l'itération", iter, "\n")
    # On stocke aussi la dernière itération si pas déjà fait
    if (iter %% pas != 0) {
      alpha_list[[compteur]] = alpha_est
      mu_list[[compteur]]    = mu_est
      var_list[[compteur]]   = var_est
...
plot(x_seq, gauss(x_seq, mu_list[[1]][1], var_list[[1]][1]),
     ...
     xlab = "x", ylab = "Densité",
     main = "Densités estimées à différentes itérations de l'EM",
     ylim = c(0, 1))

# Pour chaque étape, on trace les 3 composantes
for (j in 1:n_etapes) {
  for (k in 1:K) {
    lines(x_seq, gauss(x_seq, mu_list[[j]][k], var_list[[j]][k]),
          col = couleurs_etapes[j], lwd = 1.5)
  }
}

# Vraies composantes en rouge pointillé
for (k in 1:K) {
  lines(x_seq, gauss(x_seq, mu_vrai[k], var_vrai[k]),
        col = "red", lwd = 2, lty = 2)
}
